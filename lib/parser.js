...
const parser = (cellText) => {
  ...
  console.log(`Results for ${cellText}`);
  ...
  for (let i = 0; i < nodes.length; i++) {
    ...
    if(/\S/.test(nodes[i].textContent)) {
      ...
      // off by 8 (2x marker length)
      let startOffset = (markerLength * 2) * offsetMultiplier;
      let endOffset = markerLength * offsetMultiplier;

      console.log(`Iteration #${i} Offset: ${startOffset}, ${endOffset}`);
